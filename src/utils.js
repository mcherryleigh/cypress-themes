...
const getSourceFolder = () => {
  ...
  const installedAsFile = Cypress._.startsWith(__dirname, '/../themes');
...
const getRootTest = test => (test.parent ? getRootTest(test.parent) : test);

const hasTestFailed = test => test.state === 'failed';

const hasSuiteFailed = (suite) => {
  const failed = suite.tests.some(hasTestFailed) || suite.suites.some(hasSuiteFailed);
  return failed;
};
...
const loadTheme = (theme) => {
  ...
  if (!knownThemes.includes(usedTheme)) {
    console.error(
    ...
    usedTheme = 'dark';
    console.error('using default theme "%s"', usedTheme);
  }

  const themeFilename = join(getSourceFolder(), 'themes', `${theme}.css`);
